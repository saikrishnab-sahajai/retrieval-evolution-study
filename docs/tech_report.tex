...
\begin{description}
  ...
  \item[Replication]  Full source available at
        \url{https://github.com/saikrishnab-sahajai/retrieval-evolution-study}.
        The repository includes all scripts (\texttt{scripts/}), result files
        (\texttt{results/} as JSON), notebooks (\texttt{notebooks/}), and
        query/relevance data.  Large corpus files and model checkpoints are
        excluded but can be reproduced via \texttt{scripts/download\_datasets.py}
        and \texttt{scripts/run\_finetune.py}.
\end{description}
...
\begin{table}[h]
\centering
\caption{Passage-length statistics per dataset (words).}
...
%% Query -> retrievers (right then angle up/down)
\draw[arr] (q.east) -- ++(0.4,0) |- (bm25.west);
\draw[arr] (q.east) -- ++(0.4,0) |- (bge.west);

%% Retrievers -> RRF (horizontal label segment, then vertical join)
\draw[arr] (bm25.east) -- node[slbl, above] {top-100}
           (rrf.west |- bm25.east) -- (rrf.west);
\draw[arr] (bge.east)  -- node[slbl, below] {top-100}
           (rrf.west |- bge.east)  -- (rrf.west);
